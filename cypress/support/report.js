...
function addScreenshots() {
    ...
    const featuresList = Array.from(new Set(screenshots.map(x => x.match(/([a-zA-Z]+( [a-zA-Z]+)+)\.feature/g)[0])))
        ...
        screenshots.forEach(screenshot => {
            ...
            myScenarios.forEach(myScenario => {
                ...
                if (myScenario.steps.find(
                    ...
                    myStep = myScenario.steps.slice(-1)[0]
                }
                if (!myStep) {
                    return
                }
...
function generateReport() {
    if (!fs.existsSync(cucumberJsonDir)) {
        ...
        report.generate({
            ...
            pageTitle: 'Acquia Campaign Studio -Regression Suite Report',
            reportName: `Acquia Campaign Studio -Regression Suite Report - ${new Date().toLocaleString()}`,
            hideMetadata: true,
