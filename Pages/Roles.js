...
class Roles {
  ...
  get clickOnFirstRole() {
    return cy.get(
      'table[class="table table-hover table-striped table-bordered role-list"]>tbody>tr>td>a'
    );
  }
  ...
  get fullSystemAccess_No() {
    return cy.get(
      'label[class="btn btn-default  active btn-no btn-danger"]>span'
    );
  }
  ...
  get apiAccessGrantedCheckBox() {
    return cy
      .get('div[class="pt-md pr-md pl-md pb-md"]>div>div>div>div>label>input')
      .eq(0);
  }

  get clientApplicationsAccess() {
    return cy.get('div[class="pt-md pr-md pl-md pb-md"]>div>div>div>div>label>input')
    .eq(5);
  }
